...
Step 4 variant of the BB lecture workflow: same as `/nrs-write-chapter`, but written in first person as His Holiness Niranjana Swami's own authorial voice (he is writing the book himself), calibrated against the tone/delivery profile built from his *Collected Letters*.

`/nrs-write-chapter` (plain third-person book-author prose) is kept as a separate reference version — this is an alternative, not a replacement.
